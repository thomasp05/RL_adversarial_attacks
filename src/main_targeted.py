...
for i in range(10):
    ...
    for episode in range(nb_episode): 
        state = env.reset(attack_label=target_label)
        reward = [] 
        loss = []
        episode_done = False 
        nb_iteration = 0
        ...
        while not episode_done and max_iter <100: 
            ...
            original_image = env.original_image.to("cpu").detach().view(784)
            noise = (torch.rand(action.shape) * (image.view(784) - original_image)) * epsilon # / (nb_iteration + 1)
    ...
    print("Number of predictions equal to {0} : {1}".format(i, good_label))
    ...
    with open(textFileName, "a") as text_file:
        ...
        text_file.write("Number of predictions equal to {0} : {1}\n".format(i, good_label))
        text_file.write("\n")
